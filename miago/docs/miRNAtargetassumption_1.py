...
updateFile = open('miRNAtarget_assumption_class_1.owl','a')
...
for j in range(0,len0):
    ...
    if searchObj:
        ...
        for i in range(len(assay_list)):
            ...
            updateFile1.write('    <!-- http://purl.obolibrary.org/obo/MIAGO_'+str(assay_id)+' -->\n')
            updateFile1.write('    <owl:NamedIndividual rdf:about="&obo;MIAGO_'+str(assay_id)+'">\n        <rdf:type rdf:resource="&obo;OBI_0000070"/>\n        <rdfs:label xml:lang="en">'+str(assay)+' '+str(assay_id)+'</rdfs:label>\n    </owl:NamedIndividual>\n\n\n')
    else:
        varID = varID +1
        len1 = int(math.log10(varID)) + 1
        string_val = "0" * (7-len1)
    ...
    if as_downstream_conclusion[j] <> 'None':
        ...
        updateFile1.write('        <rdf:type rdf:resource="&obo;'+str(mother_term)+'"/>\n')
        updateFile1.write('        <rdfs:label xml:lang="en">'+as_downstream_conclusion[j]+'</rdfs:label>\n')
        updateFile1.write('        <obo:BFO_0000050 rdf:resource="&obo;MIAGO_'+str(pmid_ID)+'"/>\n')
        if assay_id_list :
            for q in range(len(assay_id_list)):
                updateFile1.write('        <obo:MIAGO_0000104 rdf:resource="&obo;MIAGO_'+assay_id_list[q]+'"/>\n')
        else:
            updateFile1.write('        <obo:MIAGO_0000104 rdf:resource="&obo;MIAGO_'+assay_id+'"/>\n')
    ...
    if as_upstream_conclusion[j] <> 'None':
        ...
        updateFile1.write('    <!-- http://purl.obolibrary.org/obo/MIAGO_'+ upconclusion_ID +' -->\n\n')
        updateFile1.write('    <owl:NamedIndividual rdf:about="&obo;MIAGO_'+ upconclusion_ID +'">\n')
        updateFile1.write('        <rdf:type rdf:resource="&obo;MIAGO_0000014"/>\n')
        updateFile1.write('        <rdfs:label xml:lang="en">'+as_upstream_conclusion[j]+'</rdfs:label>\n')
        updateFile1.write('        <obo:BFO_0000050 rdf:resource="&obo;MIAGO_'+str(pmid_ID)+'"/>\n')
        if assay_id_list :
            for q in range(len(assay_id_list)):
                updateFile1.write('        <obo:MIAGO_0000104 rdf:resource="&obo;MIAGO_'+assay_id_list[q]+'"/>\n')
    ...
    if as_coexpression[j] == 'Y':
        ...
        updateFile1.write('    <!-- http://purl.obolibrary.org/obo/MIAGO_'+ coexpression_ID +' -->\n\n')
        updateFile1.write('    <owl:NamedIndividual rdf:about="&obo;MIAGO_'+ coexpression_ID +'">\n')
        updateFile1.write('        <rdf:type rdf:resource="&obo;MIAGO_0000013"/>\n')
        updateFile1.write('        <rdfs:label xml:lang="en">'+as_miRNA_name[j]+' and '+as_target_name[j]+' co-expression conclusion</rdfs:label>\n')
        updateFile1.write('        <obo:BFO_0000050 rdf:resource="&obo;MIAGO_'+str(pmid_ID)+'"/>\n')
        if assay_id_list :
            for q in range(len(assay_id_list)):
                updateFile1.write('        <obo:MIAGO_0000104 rdf:resource="&obo;MIAGO_'+assay_id_list[q]+'"/>\n')
    ...
    if as_prediction[j] <> 'None':
        ...
        updateFile1.write('    <!-- http://purl.obolibrary.org/obo/MIAGO_'+ prediction_ID +' -->\n\n')
        updateFile1.write('    <owl:NamedIndividual rdf:about="&obo;MIAGO_' + prediction_ID +'">\n')
        updateFile1.write('        <rdf:type rdf:resource="&obo;MIAGO_0000046"/>\n')
        updateFile1.write('        <rdfs:label xml:lang="en">'+as_prediction[j]+' predicted miRNA target conclusion'+prediction_ID+'</rdfs:label>\n')
        updateFile1.write('        <obo:BFO_0000050 rdf:resource="&obo;MIAGO_'+str(pmid_ID)+'"/>\n')
        updateFile1.write('    </owl:NamedIndividual>\n\n\n')
